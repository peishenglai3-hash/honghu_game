...
function clampDevPlayerTuning(value: number): number {
	return Math.min(
		DEV_PLAYER_TUNING_MAX,
		Math.max(DEV_PLAYER_TUNING_MIN, value),
	);
}

function normalizedDevPlayerTuning(
	value: unknown,
	fallback = DEV_PLAYER_TUNING_DEFAULT,
): number {
...
function inferTaskGuidance(task: TaskCard): string | undefined {
	...
	// 第二章的正厅、渔民和第一章的敞开木门都可能出现“门边”，
	// 但它们对应的是不同的交互对象；标题优先能避免提示串线。
	const guidanceRules: Array<[RegExp, string]> = [
		[
			/名字留在纸上.*查看纪念碑|查看纪念碑、笔记或采访设备/,
			"查看纪念碑、笔记和采访设备",
		],
		[
			/名字留在纸上|纪念碑附近|碑文|确认笔记中的那个名字/,
			"前往纪念碑查看碑文与姓名",
		],
...
export type SceneOverlay =
	"Scene1Overlay" | "Scene2Overlay" | "Scene3Overlay" | null;
...
export const useHudStore = defineStore("hud", () => {
	...
	const taskWindowCount = computed(() =>
		Math.max(1, taskCards.value.length - 2),
	);
	...
	function applyDevPlayerMotionFromJson(config?: DevPlayerMotionConfig) {
		devPlayerTuning.movementMultiplier = normalizedDevPlayerTuning(
			config?.movement_multiplier,
		);
		devPlayerTuning.animationMultiplier = normalizedDevPlayerTuning(
			config?.animation_multiplier,
		);
	}

	function devPlayerMotionJson(): DevPlayerMotionConfig {
		return {
			movement_multiplier: normalizedDevPlayerTuning(
				devPlayerTuning.movementMultiplier,
			),
			animation_multiplier: normalizedDevPlayerTuning(
				devPlayerTuning.animationMultiplier,
			),
		};
	}
	...
	function _renderCurrentEntry() {
		...
		const cps = Math.max(
			4,
			Math.round((entry.cps || 14) * gameSave.getTextSpeedMult()),
		);
	...
	function playNarrative(entries: NarrativeEntry[], onComplete?: () => void) {
		// 叙事队列必须逐条保留作者条目。presentation_group 只作为内容标注，
		// 不在 HUD 核心层合并，避免丢失 entry_id、停顿、头像、音效及推进节奏。
		_narrativeQueue = entries.map((entry) => ({ ...entry }));
		_narrativeIndex = 0;
	...
	function closeItem() {
		...
		// 物品面板现在统一提供 E/点击关闭；若它是在旁白或对白期间
		// 被关闭，不能因此解除剧情锁，只能在可探索状态恢复移动。
		if (["explore", "leave_walk"].includes(gameState.state.mode)) {
			playerLocked.value = false;
		}
	}
	...
	function showTask(task: TaskCard, centerWhenEmpty = true) {
		...
		const entry = {
			...task,
			guidance: task.guidance || inferTaskGuidance(task),
			id: ++_taskId,
		};
	...
	function closeTask() {
		...
		if (taskCenterId.value !== null) {
			...
			gameState.state.playerLocked = ["explore", "leave_walk"].includes(
